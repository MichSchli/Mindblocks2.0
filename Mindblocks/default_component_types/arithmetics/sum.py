...
class Sum(ComponentTypeModel):
    ...
    def initialize_value(self, value_dictionary, language):
        return SumValue(int(value_dictionary["axis"][0][0]))
    ...
    def execute(self, execution_component, input_dictionary, value, output_value_models, mode):
        ...
        if all_lengths[value.axis] is not None:
            ...
            summed_dim = tf.reduce_sum(val, axis=value.axis)
        ...
        output_value_models["output"].assign(summed_dim, length_list=new_lengths)
...
class SumValue(ExecutionComponentValueModel):
